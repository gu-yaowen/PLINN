...
from .gnn import GNN, GPS
from .aggr import PromptAggr


class LigGNN(nn.Module):
    ...
    def get_representations(self, data, channel_idx=-1, return_score=False):
        ...
        if not self.use_prompt:
            ...
            if channel_idx > -1:
                ...
                if return_score:
                    return torch.stack(graph_reps), scores, batch_x, batch_mask
                else:
                    return torch.stack(graph_reps), None, batch_x, batch_mask
    
    def forward(self, data, channel_idx=-1):
        graph_reps, _, node_repres, batch_mask = self.get_representations(data, channel_idx=channel_idx)

        output = {}
        if not self.use_prompt or channel_idx > -1:
            output['lig_graph_rep'] = graph_reps
            output['lig_node_rep'] = node_repres
            output['mask_L'] = batch_mask
        else:
            ...
            output['lig_graph_rep'] = graph_rep
            output['lig_node_rep'] = node_repres
            output['mask_L'] = batch_mask
        return output
